...
String.prototype.isMobileNum = function() {
	/*return (new RegExp(/^((13[0-9])|(14[4,7])|(15[^4,\D])|(17[0-8])|(18[0-9]))(\d{8})$/)
			.test(this));*/
 ...
 * @param imageElementId
 */
function deleteImage(fileElementId,imageElementId) {
	$.ajax({
		url : http_path + "/FileUpload/deleteFile",
		data : {
			fileElementId : fileElementId
		},
		success : function(data) {
...
function showHighLightFront(index){
	$("#child" + index).parent().attr('class','click');
}
...
function showHighLightFront2(maxIndex, nowIndex){
	...
	var more = $("#li_" + nowIndex +" ul");
	var tg = $("#tg_" + nowIndex);
	
	li.attr('class','clicks'); 
	more.show();
...
$(function(){
	window.console.info('%c钱上钱 ', 'background-image:-webkit-gradient( linear, left top, right top, color-stop(0, #f22), color-stop(0.15, #f2f), color-stop(0.3, #22f), color-stop(0.45, #2ff), color-stop(0.6, #2f2),color-stop(0.75, #2f2), color-stop(0.9, #ff2), color-stop(1, #f22) );color:transparent;-webkit-background-clip: text;font-size:5em;');
	window.console.log("%c", "padding:50px 300px;line-height:120px;background:url('http://wayou.github.io/2014/09/10/chrome-console-tips-and-tricks/rabbit.gif') ;");
});
